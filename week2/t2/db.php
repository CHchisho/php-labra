...
$pdo->exec(<<<SQL
CREATE TABLE IF NOT EXISTS UsersTask (
...
$stmt = $pdo->prepare('SELECT user_id FROM UsersTask WHERE username = :u LIMIT 1');
$stmt->execute([':u' => 'admin']);
$admin = $stmt->fetch();
if (!$admin) {
    $hash = password_hash('admin', PASSWORD_DEFAULT);
    $ins = $pdo->prepare('INSERT INTO UsersTask (username, password_hash, role) VALUES (:u, :p, :r)');
